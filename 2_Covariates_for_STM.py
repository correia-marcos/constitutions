...
one CPP dataset. However, there seems to be something wrong in the dataset.
For instance, in 1987, Afghanistan creates a new constitution, but the new
length only happens 3 year later. Such issues happened 145 times in our
dataset.
...
and the one in the dataset. The amount of pre-processment reduced greatly the
variance of the text so this isn't a good proxy also.
We them thought about taking the amount of words in .txt text we had and
...
def get_codes(list_countries):
    """Use pycountry package to get codes of countries from dataset.
...
def get_amendments(cpp_data_df):
    """Calculate total amount of amendments (evnttype=1) for each country."""
